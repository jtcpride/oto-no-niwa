...
- **v0.16.0「相手を倒す体力バトル」**
- 本編は時間制限・返球数で終了せず、相手の体力0で勝利、自分の体力0で敗北
- 正しい返球が相手に届くと10ダメージ、PERFECTなら15ダメージ。練習は双方ダメージなし
- 開始画面の説明とプレイ中の飾り文を減らし、iPhoneではダイアログをステージ下側にコンパクトに表示
- 横タイミングバーを反転し、相手側を右・蹴りどきを左（自分側）に配置
- 1ステージ4音: `/θ/` `/ð/` `/ʃ/` `/ʒ/`
